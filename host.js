...
import { getMinutesDiff } from "./utils.js";
...
class Host {
  constructor(predefiniton = {}) {
    this.sexe = predefiniton.sexe
      ? predefiniton.sexe
      : getRandomValueInArray(sexes);
    this.name = predefiniton.name
      ? predefiniton.name
      : this.generateName(this.sexe);
    ...
    this.activities = activities.filter((activity) => {
      ...
      return (
        this.age > activity.ageLimit.min && this.age < activity.ageLimit.max
      );
    });
  ...
  alive() {
    ...
    window.cycles.alive.interval = setInterval(
      function () {
        this.breath();
      }.bind(this),
      window.speed
  ...
  wakeUp() {
    ...
    window.cycles.awake.interval = setInterval(
      function () {
        this.think();
        this.do();
        if (getRandomBoolean()) {
  ...
  sleep() {
    this.setDOMenvironment();
    this.status = "ASLEEP";
    window.cycles.asleep.interval = setInterval(
      function () {
  ...
  think() {
    ...
    personalities.forEach((personalityKey) => {
      let { value, gradient } =
        this.body.head.brain.personality[personalityKey];
      let newValue = getRandomBoolean() ? value - gradient : value + gradient;
      if (newValue > 100)
        this.body.head.brain.personality[personalityKey].value = 100;
      else if (newValue < 0)
        this.body.head.brain.personality[personalityKey].value = 0;
  ...
  do() {
    ...
    const {
      ...
      culture,
      name,
    } = getRandomValueInArray(this.activities);
    console.log(name);
  ...
  kill() {
    ...
    console.log(
      this.name +
        " is dead after " +
        getMinutesDiff(this.bornTime, this.deadTime) +
        " seconds"
    );
